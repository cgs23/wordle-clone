...
const Keyboard: React.FunctionComponent = () => {
    ...
    const isWordValid: boolean = React.useMemo(() => {
        const guess = gridState.rows[gridState.currentRow].tiles.map(x => x.character).join('');
    ...
    const buttonPressHandle = (button:string) => {
        switch(button){
            case "<<":
                ...
                break;
            case "Enter":
                if(!isSubmitable && gridState.gameStatus === GameStatus.PLAYING) toast('Please fill all the letters')
